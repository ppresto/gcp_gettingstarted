...
if [[ ${status} == "Running" ]]; then
  ...
  #Jenkins is running at http://localhost:8081 (only accessible from within the container)
...
#kubectl doesnt support 0.0.0.0 yet so http://localhost:8081 only works within container
echo "Enable port-forward with socat.  Access Jenkins at http://localhost:8080"
